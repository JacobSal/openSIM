...
if __name__ == "__main__":
    ...
    global model, initial_state, all_distances, all_candsols
    perturb = [-10000,-5000,-1000,0,1000,5000,10000]
    for i in range(0,len(perturb)):
        ...
        modSavePaths =join(prjPath,f'DynamicWalkerModel_stiffness_{perturb[i]}.osim')
        modOptSavePath = join(prjPath,f'DynamicWalkerModel_optimized_stiffness_{perturb[i]}.osim')
        all_distances = []
        all_candsols  = []

        # Load OpenSim model.
        model = osim.Model(modSavePaths)
        ...
        model.printToXML(modOptSavePath)
#endif